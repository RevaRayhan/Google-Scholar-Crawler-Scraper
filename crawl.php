                                ...
                                if (isset($_POST['crawls'])) {
                                    ...
                                    if ($con->connect_errno) {
                                        ...
                                        foreach ($html->find('div[class="gs_ri]') as $news) {
                                            ...
                                            $html2 = file_get_html("https://scholar.google.com" . $docLink); //masuk ke halaman author 
                                            $abstract = "";
                                            foreach ($html2->find('tr[class="gsc_a_tr"]') as $doc) { //masuk ke judul2 author

                                                $title = $doc->find('a', 0)->plaintext;
                                                // if (strtolower(str_replace(":", "", $docTitle)) == strtolower($title)) {
                                                if (strtolower($docTitle) == strtolower($title)) {
                                                    $link = $doc->find('a', 0)->href; //dapetin halaman judul author
